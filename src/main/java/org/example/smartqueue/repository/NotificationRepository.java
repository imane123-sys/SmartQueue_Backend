...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Query("SELECT n FROM Notification n WHERE n.ticket.client.id = :clientId ORDER BY n.dateEnvoi DESC")
    Page<Notification> findByTicketClientId(@Param("clientId") Long clientId, Pageable pageable);

    @Query("SELECT n FROM Notification n WHERE n.ticket.services.etablissement.id = :etablissementId ORDER BY n.dateEnvoi DESC")
